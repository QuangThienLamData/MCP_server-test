...
> 1. **Crawl trang JS bằng Jina Reader** (`https://r.jina.ai/`), **KHÔNG dùng Playwright** — Render giới hạn 512MB, Chromium sẽ OOM (xem commit `2bf7837`). Pattern này đã có trong `modules/rag.py`.
...
uv run python -m modules.rag # chạy 1 server standalone qua stdio (có __main__ block) để test cục bộ
...
**Pattern host + mounted sub-app** (đã có trong repo): mỗi MCP server là 1 module export `mcp = FastMCP(...)` với các `@mcp.tool()`. `main.py` mount `streamable_http_app()` của từng server dưới path prefix và chạy session manager qua `lifespan`. Hiện có: `modules/rag.py`, `modules/news.py`, `modules/reviews.py`, `modules/youtube.py`, `modules/facebook.py`, `modules/tiktok.py`, `modules/refero.py`, `modules/kg.py` — tất cả đăng ký qua `modules/research.py`.

**Thêm 1 MCP server mới** (bắt buộc đủ 3 bước để qua được Auth):
1. Tạo `modules/foo.py`: `mcp = FastMCP(name=..., transport_security=TransportSecuritySettings(enable_dns_rebinding_protection=False))` + các `@mcp.tool()`.
...
- **Embedding**: OpenAI `text-embedding-3-small` (1536 dims). Có sẵn `_embed()` trong `modules/rag.py`.
...
- **Dedup**: content-hash MD5 lưu trong SQLite `crawled_pages` (đã có trong `modules/rag.py`), skip trang không đổi.
...
## Sub A — Strategy Research RAG · `modules/rag.py` · mount `/rag` · namespace mặc định

Crawl nội dung đối thủ (`config/competitors.json`: ZaloPay/ShopeePay/VNPay…) → chunk → embed → Pinecone + dedup SQLite. **Đã có + đã bổ sung cross-lingual & Tavily fallback.**
...
- **Cross-lingual**: `search_competitor_content` dùng `_bilingual_queries` (đã chuyển xuống `modules/rag.py` làm hàm chung, `modules.news` import lại — tránh circular) → embed EN+VI → merge score max. Test thật: VI "khuyến mãi ShopeePay" (score 0.72) và **EN "ZaloPay promotions" chạm đúng nội dung tiếng Việt** (0.70).
...
## Sub B — App Store Rating RAG · `modules/reviews.py` · mount `/reviews` · namespace `app_reviews`
...
Apps trong `config/apps.json` (android + iOS, đã verify hợp lệ trên kho VN):
...
## Sub C — Refero UX Integration · `modules/refero.py` · mount `/ux`
...
Theo dõi fintech/AI/product/growth từ GNews. **Đã implement + test end-to-end trong `modules/news.py`** (mount `/news`), tái dùng `_embed`/`_get_index`/`DB_PATH` từ `modules/rag.py`. Topic/keyword cấu hình trong `config/news_topics.json` (JSON cho đồng bộ với `config/competitors.json`, không thêm dep PyYAML). Semantic search dùng Pinecone namespace `news_trends`; listing/digest dùng bảng SQLite `news_articles` (dedup theo `article_url`). GNews free plan cắt `content` ~200 ký tự → embed `title + description + content`; cần full thì `tavily.extract(url)`. Lưu ý: `/gnews` cũ vẫn còn và trùng chức năng realtime với `/news` — cân nhắc gỡ sau.
...
- `config/news_topics.json` schema `queries: [{q, lang, country?}]` (EN + VI).
...
- `scripts/cron_trigger.py` (stdlib) = lệnh cron chạy: `uv run python scripts/cron_trigger.py <target>`; đọc `WEB_SERVICE_URL` + `CRON_SECRET`.
...
1. Hạ tầng dùng chung: đã có `_embed` / `_get_index` / Jina Reader / dedup trong `modules/rag.py` — tách ra dùng chung nếu cần; thêm `tavily_client`.
2. **Sub D** (gần xong nhất: chỉ thêm index + digest quanh `modules/news.py`).
3. **Sub A** (mở rộng `modules/rag.py` + Tavily fallback).
